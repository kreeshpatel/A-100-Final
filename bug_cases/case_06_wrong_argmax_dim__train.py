"""
BUG CASE 06: torch.max called along the wrong dimension during evaluation.

Drop-in replacement for src/train.py.

ORIGINAL : _, predicted = torch.max(outputs, 1)   # in evaluate()
MODIFIED : _, predicted = torch.max(outputs, 0)

EXPECTED FAILURE (silent):
    No crash. Training loss looks normal but reported test accuracy
    collapses to ~10% because torch.max(outputs, 0) reduces over the
    batch dimension instead of the class dimension - the returned tensor
    contains sample indices, not class predictions.
...
def evaluate(model, loader, device):
    ...
    with torch.no_grad():
        for images, labels in loader:
            images, labels = images.to(device), labels.to(device)
            outputs = model(images)
            # BUG: dim should be 1 (class axis on a [B, C] tensor). Using 0
            # reduces over the batch axis and returns sample indices, not
            # class predictions, so accuracy collapses to ~10%.
