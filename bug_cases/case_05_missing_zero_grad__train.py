"""
BUG CASE 05: optimizer.zero_grad() removed from the training step.
...
ORIGINAL : optimizer.zero_grad()
           ...
           loss.backward()
           optimizer.step()
MODIFIED : # optimizer.zero_grad() removed
           outputs = model(images)
           loss = criterion(outputs, labels)
           loss.backward()
           optimizer.step()
...
EXPECTED FAILURE (silent):
    No crash. Training loss bounces around and final accuracy is far below
    baseline (~30-50% instead of ~98.8%) because gradients accumulate
    across batches and Adam takes huge, unstable steps.

REPRODUCE:
    cp bug_cases/case_05_missing_zero_grad__train.py src/train.py
    python src/train.py
"""
...
def train_one_epoch(model, loader, criterion, optimizer, device):
    ...
    for images, labels in loader:
        ...
        # BUG: optimizer.zero_grad() is missing. Without it, .grad on every
        # parameter accumulates across all prior batches, so optimizer.step()
        # uses a stale composite gradient instead of the current batch's.
        outputs = model(images)
        loss = criterion(outputs, labels)
        loss.backward()
        optimizer.step()
