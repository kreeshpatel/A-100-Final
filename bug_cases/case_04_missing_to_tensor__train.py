"""
BUG CASE 04: transforms.ToTensor() removed from the data pipeline.

Drop-in replacement for src/train.py.

ORIGINAL : transform = transforms.Compose([
               transforms.ToTensor(),
               transforms.Normalize((0.1307,), (0.3081,)),
           ])
MODIFIED : transform = transforms.Compose([
               transforms.Normalize((0.1307,), (0.3081,)),
           ])
...
REPRODUCE:
    cp bug_cases/case_04_missing_to_tensor__train.py src/train.py
    python src/train.py
"""
...
def get_data_loaders(batch_size):
    # BUG: ToTensor() was removed. Normalize requires a Tensor input,
    # but datasets.MNIST yields PIL images by default, so the very first
    # transform call raises TypeError.
