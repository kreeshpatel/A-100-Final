"""
BUG CASE 10: Training DataLoader has shuffle=False.

Drop-in replacement for src/train.py.

ORIGINAL : DataLoader(train_dataset, ..., shuffle=True,  ...)
MODIFIED : DataLoader(train_dataset, ..., shuffle=False, ...)

EXPECTED FAILURE (silent):
    No crash. Final test accuracy drops from ~98.8% baseline to mid-90s,
    and the loss curve looks less smooth. Mini-batch optimizers assume
    each batch is roughly an i.i.d. sample of the data; fixed-order
    batches break that assumption and give the optimizer a less helpful
    signal each epoch.

REPRODUCE:
    cp bug_cases/case_10_no_train_shuffle__train.py src/train.py
    python src/train.py
"""
...
def get_data_loaders(batch_size):
    ...
    # BUG: shuffle=False on the *training* loader. Test can stay unshuffled
    # because order does not matter for evaluation, but training relies on
    # randomized batch order.
