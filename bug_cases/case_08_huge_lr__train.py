"""
BUG CASE 08: Learning rate set 10000x too large.

Drop-in replacement for src/train.py.

ORIGINAL : --lr default = 0.001
MODIFIED : --lr default = 10.0

EXPECTED FAILURE (silent):
    Loss prints as nan after a handful of iterations; final accuracy is
    ~10% (random). Updates of magnitude lr * grad blow parameters into
    overflow regions on the very first step; subsequent forward passes
    produce inf/nan, which propagates everywhere.

REPRODUCE:
    cp bug_cases/case_08_huge_lr__train.py src/train.py
    python src/train.py
"""
...
def parse_args():
    ...
    # BUG: 10.0 is wildly too large for Adam on this problem. Reasonable
    # values are in the 1e-4 to 1e-3 range. Even one update overflows.
